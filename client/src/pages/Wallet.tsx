...
export default function Wallet() {
    ...
    // Read current balance from whatever we have locally.
    // TODO: once you build GET /api/v1/auth/me (or similar), replace this with a fetch
    // that returns the live bcoins balance, instead of relying on cached localStorage data.
    useEffect(() => {
        const stored = localStorage.getItem('user')
        if (stored) {
            try {
                const user = JSON.parse(stored)
                if (typeof user.bcoins === 'number') {
                    setBcoins(user.bcoins)
                }
            } catch {
                // ignore parse errors
            }
                ...
                handler: async (response: any) => {
                    ...
                    try {
                        ...
                        if (verifyData.success) {
                            const updatedBcoins = verifyData.data?.user?.bcoins
                            if (typeof updatedBcoins === 'number') {
                                setBcoins(updatedBcoins)

                                // keep localStorage in sync until a /me endpoint exists
                                const stored = localStorage.getItem('user')
                                if (stored) {
                                    const user = JSON.parse(stored)
                                    user.bcoins = updatedBcoins
        ...
        <Layout>
            <section className="px-4 sm:px-8 lg:px-16 py-16 lg:py-24 max-w-4xl mx-auto">
                ...
                <div className="bg-[#11112A] border border-[rgba(0,191,255,0.15)] rounded-xl p-6 sm:p-8">
                    ...
                    {message && (
                        <div
                            className={`flex items-start gap-2.5 px-4 py-3 rounded-lg text-sm font-medium border mb-6 ${message.type === 'success'
